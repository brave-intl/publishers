...
export default function Preview({ isOpen, channel }) {
  ...
  const social = t(
    `contribution_pages.channel_names.${channel.details_type.split('ChannelDetails').join('').toLowerCase()}`,
  );
  ...
  return isRendered ? (
    <div className='container mx-auto min-h-[880px] w-full min-w-[640px] py-4 md:min-w-[768px] lg:min-w-[1024px] xl:min-w-[1280px]'>
      <h3 className='mb-4 pl-4'>
        {t('contribution_pages.preview_title', { social, title })}
      </h3>
      <PublicChannelPage
        publicIdentifier={channel.public_identifier}
        previewMode={true}
      />
    </div>
  ) : null;
