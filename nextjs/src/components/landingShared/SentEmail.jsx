...
export default function SentEmail({ tryAgain, words }) {
  ...
  return (
    <>
      <div
        className={`${styles['box']} ${styles['bat-animation']} mb-30px flex-col`}
      >
        <svg height='160px'>
          <BatLockup />
        </svg>
      </div>
      <div className={`${styles['box']} flex-center w-[600px] flex-col`}>
        <h2 className={`${styles['sign-title']} m-[24px]`}>{words.headline}</h2>
        <p className={`${styles['email-sent-text']}`}>
          {words.body}
          <a
            rel='noopener'
            className='cursor-pointer text-white opacity-80'
            onClick={tryAgain}
          >
        ...
        <div
          className={`${styles['box']} flex-row ${styles['sent-email-icon-container']}`}
        >
          <a
            rel='noopener'
