...
function TextBlock({ side, step, title, description }) {
  return (
    <div
      className={`${styles['box']} min-h-[initial] w-1/2 flex-row p-[24px] ${styles[side]}`}
    >
      <h1 className={`${styles['summary-number']}`}>{step}</h1>
      <div className='flex-[0 0 auto] w-[12px] self-stretch md:w-[16px]'></div>
      <div className={`${styles['box']} w-auto flex-col md:w-[480px]`}>
        <h3 className={`${styles['spotlight-heading']} m-0 text-white`}>
          {title}
        </h3>
        <p className='w-max-[600px] my-[1em] leading-[1.6] text-[#E9E9F4] md:text-[16px] md:text-white'>
          {description}
        </p>
      </div>
    </div>
...
export default function Summary() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`${styles['box']} w-full max-w-[1200px] flex-col px-[48px]`}
      >
        ...
        />
        <div className='my-[24px] w-[70%] border border-solid border-[#ffffff26]'></div>
        <div
          className={`${styles['box']} my-[24px] w-full flex-row justify-center`}
        >
          <CardButton
            href={t('landingPages.summary.cardBusinessHref')}
            icon={
              <Icon
                name='message-bubble-chat'
