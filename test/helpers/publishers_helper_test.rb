...
class PublishersHelperTest < ActionView::TestCase
  ...
  test "publisher_owner_converted_balance should balance in BAT for unset publisher currency" do
    publisher = publishers(:default)
    publisher.default_currency = nil
    publisher.save
    assert_dom_equal %{}, publisher_owner_converted_balance(publisher)
  end

  test "publisher_owner_converted_balance should return nothing for BAT publisher currency" do
    publisher = publishers(:default)
  ...
  test "publisher_converted_balance should return something for set publisher currency" do
    ...
    publisher.save
    assert_dom_equal %{~ 0.00 USD}, publisher_owner_converted_balance(publisher)
  end
  ...
  test "publisher_converted_balance should return `CURRENCY unavailable` when no wallet is set" do
    class FakePublisher
      ...
      def initialize(wallet_json:, channels_json:)
        @wallet = Eyeshade::Wallet.new(wallet_json: wallet_json, channels_json: channels_json) if wallet_json
    ...
    publisher = FakePublisher.new(
      ...
      channels_json: [
        {
          "account" => "youtube#channel:abc123",
          "balance" => "9001"
        }
      ]
    )
    assert_not_nil publisher.wallet
    assert_dom_equal %{~ 2127.71 USD}, publisher_owner_converted_balance(publisher)
    ...
    assert_equal "USD unavailable", publisher_owner_converted_balance(publisher)
  end
