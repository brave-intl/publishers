...
module Publishers
  class AbstractOmniauthCallbacksControllerTest < ActionDispatch::LegacyIntegrationTest
    ...
    before(:example) do
      OmniAuth.config.test_mode = true
      @active_promo_id_original = Rails.application.credentials[:active_promo_id]
      Rails.application.credentials[:active_promo_id] = ""
      uphold_url = Rails.application.credentials[:uphold_api_uri] + "/v0/me"
    ...
    after(:example) do
      Rails.application.credentials[:active_promo_id] = @active_promo_id_original
      OmniAuth.config.test_mode = false
    end
  end
