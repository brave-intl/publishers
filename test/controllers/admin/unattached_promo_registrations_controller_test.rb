...
class Admin::UnattachedPromoRegistrationsControllerTest < ActionDispatch::IntegrationTest
  ...
  before(:example) do
    @prev_offline = Rails.configuration.pub_secrets[:api_promo_base_uri] # Presence of this envar means we use external requiests
  end
  ...
  test "#create creates codes" do
    ...
    stub_request(:put, "#{Rails.configuration.pub_secrets[:api_promo_base_uri]}/api/2/promo/referral_code/unattached?number=1")
  ...
  test "#update_statuses updates the status of codes" do
    ...
    stub_request(:patch, "#{Rails.configuration.pub_secrets[:api_promo_base_uri]}/api/2/promo/referral?referral_code=ABC123")
  ...
  test "#assign_installer_type assigns installer type to codes" do
    ...
    stub_request(:put, "#{Rails.configuration.pub_secrets[:api_promo_base_uri]}/api/2/promo/referral/installerType")
      .to_return(status: 200)
