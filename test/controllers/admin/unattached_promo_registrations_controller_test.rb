...
class Admin::UnattachedPromoRegistrationsControllerTest < ActionDispatch::IntegrationTest
  ...
  before(:example) do
    @prev_offline = Rails.application.credentials[:api_promo_base_uri] # Presence of this envar means we use external requiests
  end

  after(:example) do
    Rails.application.credentials[:api_promo_base_uri] = @prev_offline
  end

  test "#create creates codes" do
    Rails.application.credentials[:api_promo_base_uri] = "http://127.0.0.1:8194" # Turn on external requests
    admin = publishers(:admin)
    sign_in admin

    stub_request(:put, "#{Rails.application.credentials[:api_promo_base_uri]}/api/2/promo/referral_code/unattached?number=1")
  ...
  test "#update_statuses updates the status of codes" do
    ...
    stub_request(:patch, "#{Rails.application.credentials[:api_promo_base_uri]}/api/2/promo/referral?referral_code=ABC123")
