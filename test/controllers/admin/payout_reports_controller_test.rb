...
class PayoutReportsControllerTest < ActionDispatch::IntegrationTest
  ...
  let(:uphold_url) { Rails.application.credentials[:uphold_api_uri] + "/v0/me" }
  ...
  test "#create doesn't send email or set final if no params are present in POST" do
    ...
    stub_request(:get, "#{Rails.application.credentials[:api_eyeshade_base_uri]}/v1/accounts/balances?account=publishers%23uuid:1a526190-7fd0-5d5e-aa4f-a04cd8550da8&account=uphold_connected.org&account=twitch%23channel:ucTw&account=twitter%23channel:def456&pending=true")
