...
class UpholdServiceTest < ActiveJob::TestCase
  before do
    ...
    @prev_fee_rate = Rails.configuration.pub_secrets[:fee_rate]
  end

  after do
    Rails.configuration.pub_secrets[:api_eyeshade_offline] = @prev_offline
    Rails.configuration.pub_secrets[:fee_rate] = @prev_fee_rate
  end
  ...
  describe "when user is blocked from uphold" do
    ...
    before do
      Rails.configuration.pub_secrets[:fee_rate] = 0.05
