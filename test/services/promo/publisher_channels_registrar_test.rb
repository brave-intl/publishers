...
class Promo::PublisherChannelsRegistrarTest < ActiveJob::TestCase
  ...
  before(:example) do
    @prev_offline = Rails.application.credentials[:api_promo_base_uri]
  end
  ...
  test "registrar requests promo code for channel if encounters duplicate error" do
    ...
    stub_request(:put, "#{Rails.application.credentials[:api_promo_base_uri]}/api/1/promo/publishers")
      .to_return(status: 409)

    # Stub Promo response with the referral code and current code owner
    stub_request(:get, "#{Rails.application.credentials[:api_promo_base_uri]}/api/2/promo/referral_code/channel/completed.org")
      .to_return(status: 200, body: {referral_code: "COM001", owner_id: "invalid"}.to_json)

    # Stub Promo response when we update the owner
    stub_request(:put, "#{Rails.application.credentials[:api_promo_base_uri]}/api/1/promo/publishers/COM001")
