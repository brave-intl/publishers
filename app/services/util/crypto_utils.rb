...
class Util::CryptoUtils
  ...
  # Ethereum signs with secp256k1 and hashes with Keccak-256
  ...
  # previous Eth ruby libraries have been slow to update and bring in unnecessary dependencies
  # signature verification here is based on https://github.com/q9f/eth.rb/blob/main/lib/eth/signature.rb
  def self.verify_ethereum_address(signature, address, message, current_publisher)
    ...
    signature_address.downcase == address.to_s.downcase
  rescue => e
    LogException.perform(e, publisher: current_publisher)
    false
  end
