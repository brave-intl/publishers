  ...
  module Models
    class Authorization < Client
      ...
      def post_to_uphold(refresh_token)
        uri = URI.parse("#{Rails.application.credentials[:uphold_api_uri]}#{PATH}")
        request = Net::HTTP::Post.new(uri)
        request.basic_auth(Rails.application.credentials[:uphold_client_id], Rails.application.credentials[:uphold_client_secret])
