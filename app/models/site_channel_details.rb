...
class SiteChannelDetails < BaseChannelDetails
  ...
  validates :brave_publisher_id, absence: true, if: -> {
                                                      T.bind(self, SiteChannelDetails)
                                                      !errors.include?(:brave_publisher_id_unnormalized) && brave_publisher_id_unnormalized.present?
                                                    }
  ...
  before_validation :register_brave_publisher_id_error, if: -> {
                                                              T.bind(self, SiteChannelDetails)
                                                              brave_publisher_id_error_code.present?
                                                            }

  # clear/register domain errors as appropriate
  before_validation :clear_brave_publisher_id_error, if: -> {
                                                           T.bind(self, SiteChannelDetails)
                                                           brave_publisher_id_unnormalized.present? && brave_publisher_id_unnormalized_changed?
                                                         }
