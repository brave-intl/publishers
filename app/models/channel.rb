...
class Channel < ApplicationRecord
  ...
  validate :site_channel_details_brave_publisher_id_unique_for_publisher, if: -> { T.bind(self, Channel); details_type == "SiteChannelDetails" }

  validate :verified_duplicate_channels_must_be_contested, if: -> { T.bind(self, Channel).verified? }

  after_save :notify_slack, if: -> { T.bind(self, Channel); :saved_change_to_verified? && verified? }
  after_save :create_deposit_id
  before_save :set_derived_brave_publisher_id, if: -> { T.bind(self, Channel); derived_brave_publisher_id.nil? }

  # *ChannelDetails get autosaved from above.
  after_save :update_site_banner_lookup!, if: -> { T.bind(self, Channel); :saved_change_to_verified? && verified? }
  after_commit :register_channel_for_promo, if: :should_register_channel_for_promo?
  after_commit :create_channel_card, if: -> { T.bind(self, Channel); :saved_change_to_verified? && verified? }
