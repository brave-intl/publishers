...
class Publisher < ApplicationRecord
  ...
  validates :email, email: true, presence: true, unless: -> {
                                                           T.bind(self, Publisher)
                                                           pending_email.present? || deleted? || browser_user?
                                                         }
  validates :pending_email, email: {strict_mode: true}, presence: true, allow_nil: true, if: -> {
                                                                                               T.bind(self, Publisher)
                                                                                               !(deleted? || browser_user?)
                                                                                             }
  validate :pending_email_must_be_a_change, unless: -> {
                                                      T.bind(self, Publisher)
                                                      deleted? || browser_user?
                                                    }
  validate :pending_email_can_not_be_in_use, unless: -> {
                                                       T.bind(self, Publisher)
                                                       deleted? || browser_user?
                                                     }
  ...
  after_update :set_onboarding_status, if: -> {
                                             T.bind(self, Publisher)
                                             email.present? && email_before_last_save.nil?
                                           }
  after_update :set_active_status, if: -> {
                                         T.bind(self, Publisher)
                                         saved_change_to_two_factor_prompted_at? && two_factor_prompted_at_before_last_save.nil?
                                       }
