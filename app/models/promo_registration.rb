...
class PromoRegistration < ApplicationRecord
  ...
  validates :channel_id, presence: true, if: -> { T.bind(self, PromoRegistration); kind == CHANNEL }
  validates :publisher_id, presence: true, unless: -> { T.bind(self, PromoRegistration); kind == UNATTACHED }
