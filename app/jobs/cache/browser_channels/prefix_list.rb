...
class Cache::BrowserChannels::PrefixList
  ...
  def save_to_s3!(temp_file_path:, save_to_filename:)
    ...
    Aws.config[:credentials] = Aws::Credentials.new(Rails.application.credentials[:s3_rewards_access_key_id], Rails.application.credentials[:s3_rewards_secret_access_key])
    s3 = Aws::S3::Resource.new(region: Rails.application.credentials[:s3_rewards_bucket_region])
    obj = s3.bucket(Rails.application.credentials[:s3_rewards_bucket_name]).object(save_to_filename)
